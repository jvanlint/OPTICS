...
def package_v2(request, link_id):
    ...
    if (
        package.mission.campaign
    ):  # fixes error in esting where package might not be connected to a campaign.
        breadcrumbs = {
            "Home": reverse("campaigns"),
            package.mission.campaign.name: reverse(
                "campaign_detail_v2", args=(package.mission.campaign.id,)
            ),
            package.mission.name: reverse("mission_v2", args=(package.mission.id,)),
            package.name: "",
        }
    else:
        breadcrumbs = {}
    ...
    return render(request, "v2/package/package.html", context=context)
