...
class Mission(models.Model):
    ...
    discord_msg_id = models.CharField(
        max_length=20, blank=True, null=True, verbose_name="Discord Msg ID"
    )
    ...
    def create_discord_event(self, image_url, request):
        ...
        register_url = request.build_absolute_uri(
            reverse("mission_signup_v2", args=(self.id,))
        )
        mission_page = request.build_absolute_uri(
            reverse("mission_v2", args=(self.id,))
        )
        ...
        if self.discord_msg_id:
            ...
            api_result = requests.patch(
                api_patch_url, json=api_data, headers=api_headers
            )
        else:
    ...
    def new(self, campaignObject, user):
